...
public class AiragAppServiceImpl extends ServiceImpl<AiragAppMapper, AiragApp> implements IAiragAppService {
    ...
    @Override
    public String getSqlText(String inputStr) {
        FlowRunParams params = new FlowRunParams();
        params.setFlowId("1940936450833772545");
        Map<String, Object> inputs = new HashMap<>();
        inputs.put("content", inputStr);
        params.setInputParams(inputs);
        params.setResponseMode("blocking");
        System.out.println("进入params设置"+params.toString());
        Result<Object> o = (Result<Object>) airagFlowService.runFlow(params);
        System.out.println("大模型返回的"+o.toString());
        String result = (String) o.getResult();
        System.out.println("生成的result"+result);
        result = result
                .replaceAll("```json", "")
                .replaceAll("```sql", "")
                .replaceAll("```", "")
                .replaceAll(";", "")
                .replaceAll("\\.","");
        System.out.println("replaceAll后的的result"+result);
//        return result;
        return JSONObject.parseObject(result).getString("sql");
    }
