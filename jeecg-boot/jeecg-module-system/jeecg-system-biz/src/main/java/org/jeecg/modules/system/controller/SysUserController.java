...
public class SysUserController {
    ...
    public Result<List<JSONObject>> textToSql(@RequestBody JSONObject jsonObject, HttpServletRequest request) {
        String inputStr = jsonObject.getString("inputStr");
        System.out.println("接收到:" + inputStr);

        // 核心逻辑：调用系统内的 API，把自然语言转成 SQL
        String sql = sysBaseAPI.getSqlText(inputStr);
        System.out.println("sql语句为: " + sql);

        // 修正SQL（去掉“sql”这个多余文字）
        sql = sql.replace("sql", "");
        System.out.println("replace后的sql语句为: " + sql);

        // 执行SQL，获取数据
        List<JSONObject> data = sysUserService.getDataBySql(sql);
